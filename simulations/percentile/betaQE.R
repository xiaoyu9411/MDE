...
sb<-2
truemean<-sa/(sb+sa)
truesd<-sqrt((sa*sb)/((sa+sb)^2*(sa+sb+1)))
...
for (i in nnum ){
  ...
  for (j in 1:nsim){
    n<-i
    datset<-rbeta(n,shape1=sa,shape2=sb)
    xbar[j]<-mean(datset)
    sd[j]<-sd(datset)
    s<-summary(datset)
    a<-as.numeric(s[1])
  ...
  write.table(summarydat[it,],file="resultBetaQE.txt",append=TRUE,sep=",",col.names = FALSE,row.names = FALSE)
}
