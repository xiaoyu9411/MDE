...
wqe.fit<-function(min.val, q1.val, med.val, q3.val, max.val, n)
{
  ...
  residual.g<-abs(quants -pre.g)
  value.g<-sum(residual.g)
  
  valuesabs <- c(value.n, value.l, value.g,
                 value.w, value.b)
  
  names(valuesabs) <- c("normal", "log-normal", "gamma", "weibull", "beta")
  ...
  output <- list(norm.par = norm.par, lnorm.par = lnorm.par,
                 ...
                 beta.par = beta.par, valuesabs = valuesabs,
...
wqe.mean.sd <- function(min.val, q1.val, med.val, q3.val, max.val, n) {
  ...
  selected.dist <- names(which.min(x$valuesabs))
  ests <- get.mean.sd(x, selected.dist)
  output <- list(est.mean = ests$est.mean, est.sd = ests$est.sd,
                 selected.dist = selected.dist, valuesabs = x$values,args=args,
                 scenario = x$scenario, fitted.dists = x)
  for (dist.name in names(x$valuesabs)){
...
MDEx.mean.sd <- function(min.val, q1.val, med.val, q3.val, max.val, n) {
  ...
  ests <- get.mean.sd(x, selected.dist)
  output <- list(est.mean = ests$est.mean, est.sd = ests$est.sd,
                 selected.dist = selected.dist, valuesabs = x$valuesabs,args=args,
                 scenario = x$scenario, fitted.dists = x)
  for (dist.name in names(x$valuesabs)){
    ests.all <- get.mean.sd(x, dist.name)
    output[paste0(dist.name, '.est.mean')] <- ests.all$est.mean
...
for (i in nnum ){
  ...
  for (j in 1:nsim){
    ...
    #luo's method
    mluo[j]<-((4/(4+n^0.75))*((a+b)/2))+((n^0.75)/(4+n^0.75))*m
    #Shi's method
  ...
  disqe[it]<-mean(disqeall=="normal")
  diswqe[it]<-mean(diswqeall=="normal")
  dismdex[it]<-mean(dismdexall=="normal")
  ...
  write.table(summarydat[it,],file="resultnormalS1.txt",append=TRUE,sep=",",col.names = FALSE,row.names = FALSE)
}
