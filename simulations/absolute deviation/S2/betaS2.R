...
for (i in nnum ){
  ...
  for (j in 1:nsim){
    ...
    #luo's method
    mluo[j]<-8*(((0.7+0.39/n)*((q1+q3)/2))+((0.3-0.39/n)*m))
    #Shi's method
    theta2<-2*qnorm((0.75*n-0.125)/(n+0.25))
    sdshi[j]<-8*((q3-q1)/theta2)
  ...
  write.table(summarydat[it,],file="resultbetaS2abs.txt",append=TRUE,sep=",",col.names = FALSE,row.names = FALSE)
}
